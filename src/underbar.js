...
(function() {
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(item) {
      return (functionOrKey.call ? functionOrKey : item[functionOrKey]).apply(item, args);
    });
  };
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(result, item) {
      ...
      if(typeof iterator === 'function') 
        return iterator(item) ? result=true : result=false;
      else
        return item;
    }, result);
  ...
  _.defaults = function(obj) { 
    for(var i = 1; i < _.defaults.arguments.length; i++) {
        if(Object.getOwnPropertyNames(_.defaults.arguments[i]).length > 0)
          obj =  _.extend(_.defaults.arguments[i], obj);
    }   
    return obj; 
  };
  ...
  _.shuffle = function(array) {
    var arr2 = Array.prototype.slice(array);
    _.each(arr2, function(elem, index) {
